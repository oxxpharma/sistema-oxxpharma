          ...
          {results.map(p => {
            ...
            return (
              <button
                ...
                disabled={oos}
                onMouseDown={(e) => e.preventDefault()}
                onClick={() => { if (!oos) { onPick(p); setQ(''); setOpen(false); } }}
                className={`w-full text-left px-3 py-2 border-b border-border last:border-b-0 ${oos ? 'opacity-50 cursor-not-allowed bg-bg-secondary/50' : 'hover:bg-bg-secondary'}`}
                data-testid={`pdv-product-result-${p.product_id}`}
              >
                <div className="font-semibold text-sm flex items-center gap-2">
                  {p.name}
                  {oos && <span className="text-[9px] font-bold uppercase text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded">Sem estoque</span>}
                </div>
