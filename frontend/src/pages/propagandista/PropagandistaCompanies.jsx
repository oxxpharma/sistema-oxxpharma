...
import { Loader2, Building2, Users, Percent, Plus, ExternalLink } from 'lucide-react';
    ...
    <div data-testid="propagandista-companies">
      <div className="flex items-start justify-between gap-3 flex-wrap mb-6">
        <div>
          ...
          <p className="text-sm text-txt-secondary mt-1">Empresas do seu convênio · você recebe 5% da compra dos funcionários e 15% quando a empresa comprar direto</p>
        </div>
        <a href="mailto:comercial@oxxpharma.com?subject=Cadastro de nova empresa no convênio" className="inline-flex items-center gap-2 h-10 px-4 rounded-lg bg-brand-main text-white text-sm font-semibold hover:bg-brand-hover" data-testid="request-new-company">
          <Plus className="w-4 h-4" /> Solicitar nova empresa
        </a>
      </div>
      ...
      {companies.length === 0 ? (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 text-center">
          ...
          <p className="text-sm text-amber-800 mt-1">Peça pro comercial da OxxPharma vincular uma empresa a você. Você receberá comissões automaticamente sobre as compras.</p>
        </div>
      ) : (
