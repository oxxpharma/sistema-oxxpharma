...
- ✅ Iter 38 (Fev/2026): Voucher no checkout + Maxx por pedido + Pagamento full-voucher
  ...
  - **Create/Edit Usuário — seletor de papel completo:** `UserCreateModal` tinha apenas `customer`/`admin`. Agora ambos os modais (Create e Edit) exibem as 5 roles (`customer, comercial, financeiro, admin, super_admin`) com descrições. Opções `admin/super_admin` visíveis apenas para super_admin (via `useAuth().isSuperAdmin`). Backend valida: só super_admin cria ou promove a admin/super_admin; `access_level` agora é sincronizado automaticamente com a role (0 para admin/super_admin, 1 para comercial/financeiro, 99 para customer).
  - **UX modais em telas pequenas:** `flex items-center` estava cortando o topo/base em viewports baixos (ex. laptop 1024×500 ou tablet). Trocado para `items-start sm:items-center` + `my-4 sm:my-8` — em telas pequenas o modal alinha no topo e scrolla; em telas grandes mantém centralizado.
  - Testes: 20/20 passing (2 testes novos em `test_iter38_role_create_update.py`; 1 skipped).
